...
def _create_reg_file():
    if file_exists('registration_logs.json') == False:
        ...
        with open('registration_logs.json', 'w') as fp:
            fp.write(json_obj)
...
def _generateCodeAndlog(email):
    ...
    if os.stat('registration_logs.json').st_size == 0:
        json_obj = json.dumps(init_data, indent = 4)
        with open('registration_logs.json', 'w') as fp:
            fp.write(json_obj)
    else:
        with open('registration_logs.json') as fp:
            listObj = json.load(fp)
            listObj.append(init_data)
            with open('registration_logs.json', 'w') as json_file:
                json.dump(listObj, json_file, indent=4, separators=(',',': '))
    return random_number

# send mail for registration
def _sendmail(email, code = None, notify = None):
    email_creds = _creds('Dencrypt', 'credentials.json')
...
# interactive part
if file_exists('credentials.json') != True:
    print("No credentials.json exists, it should be created\n")
    _creds('Encrypt', 'credentials.json')
    Bot_Api_key = (_creds('Dencrypt', 'credentials.json')['Bot_Api_key']).replace("'", "")
else:
    Bot_Api_key = (_creds('Dencrypt', 'credentials.json')['Bot_Api_key']).replace("'", "")
...
def valid_code(auth):
    ...
    if ifregisred:
        ...
        with open('registration_logs.json', 'w') as amend_file:
...
def checkweather(period):
    Api_info = _creds('Dencrypt', 'credentials.json')
    range_ = period.text
    with open('registration_logs.json') as fp:
        listObj = json.load(fp)
